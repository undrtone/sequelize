  ...
  Query.prototype.run = function(sql) {
    ...
    var promise = new Utils.Promise(function(resolve, reject) {
      ...
      if (self.sql === 'BEGIN TRANSACTION'){
        var transaction = new self.lib.Transaction(self.connection);

        transaction.begin(function (err) {
          if (err) {
            reject(self.formatError(err));
          } else {
             self.options.transaction._transaction = transaction;
             resolve();
          }
        });
      } else {
        ...
        if (self.options.transaction && self.options.transaction._transaction) {
          // creating transaction request
          request = new self.lib.Request(self.options.transaction._transaction);
          ...
          if (self.sql === 'COMMIT TRANSACTION;' ||
            ...
            self.options.transaction._transaction[transCommand](function (err, result) {
              delete self.options.transaction._transaction;
          ...
          request = new self.lib.Request(self.connection);
        }
